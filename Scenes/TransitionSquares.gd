...
#				my_rot = deg_to_rad(-90)
			...
			tween.tween_property(square, "scale", Vector2(float(fade_to_black), float(fade_to_black)), 0.9).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUAD) #, Tween.TRANS_CIRC, Tween.EASE_OUT_IN, my_delay)
